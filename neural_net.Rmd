...
```{r}
predict_classes = nn_mod_1 %>% predict_classes(trainX)
```
...
bootstrap_accuracy = function(model){
  bootstrap_nums = sample(1:nrow(wholeX), nrow(wholeX), replace = TRUE)
  bootstrapX = wholeX[bootstrap_nums,]
  bootstrapY = wholeY[bootstrap_nums,]
